...
async function getBestMindmapIdeas(nodeList, count = 3) {
  ...
  const prompt = `
    ...
    마인드맵 노드 목록을 분석하고, 중요한 42~43자 길이의 추천 주제를 ${count}개 생성하세요.
    
    마인드맵 노드 목록:
    ${formattedNodes}
    
    ===글자 수 지침===
    • 중요: 각 주제는 공백 포함 정확히 42~43자여야 합니다. 이 지침을 엄격히 준수하세요.
    • 글자 수는 출력에 포함하지 마세요. 주제 텍스트만 작성하세요.
    • 41자 이하는 절대 허용되지 않습니다.
    ...
    ###출력 형식###
    (각 주제를 별도의 줄에 작성하고, 글자 수 표시나 부가 설명 없이 42-43자 길이의 주제만 나열하세요)
    
    위 지침에 따라 각 줄에 하나의 주제만 작성하고, 정확히 ${count}개의 주제를 나열해주세요.
    모든 주제는 반드시 42~43자(공백 포함)여야 합니다.
    
    각 주제 생성 전에 글자 수를 미리 계산하고, 정확히 42-43자가 되도록 조정하세요. 이 지침을 엄격히 준수하는 것이 매우 중요합니다.
`;
